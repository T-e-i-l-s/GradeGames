...
messages = 0
nms = ['Аккмалутдинов','Бариев','Воробьев','Галаутдинов','Гашигулин','Димухаметов','Загидулин','Закиров','Ильин',
...
bot.send_message(1118648643,"Привет уваж пользователь в этой версии мы создали что то крутое,"
                 "эта версия секретная,здесь обсолютно нет Кирилла,"
                 "теперь все работает засчет искуственного интелекта 'МегаМозгКирилла3000'"
                 "Удачи в использывании")
...
mnk = ['алго','алго','алго','алго','алго','алго','алго','алго']
tuk = ['алго','алго','алго','алго','алго','алго','алго','алго']
wdk = ['алго','алго','алго','алго','алго','алго','алго','алго']
thk = ['алго','алго','алго','алго','алго','алго','алго','алго']
frk = ['алго','алго','алго','алго','алго','алго','алго','алго']
stk = ['алго','алго','алго','алго','алго','алго']
...
def ans(message):
    ...
    StartMSG = f.read()
    f.close()
    if int(StartMSG) == 0 and message.chat.id not in cust and message.chat.id in ind:
        nmstrt = ind.index(message.chat.id)
        nmstrt = nms[nmstrt]
        custn = custn + 1
        cust[custn-1] = message.chat.id
        bot.send_message(message.chat.id, 'Привет, ' + nmstrt + '! Вас приветствует бот версии 2.0. '
                                          'Во мне все самые лучшие решения для легкой учебы и в целом порядка в классе! '
                                            'Я помогу узнать ДЗ,расписание,напоню про дежурство. '
                                          'Добавлены PUSH-уведомления,которые может отправлять староста и '
                                          'приходят они всем. Новая система анти спама защитит бота от излишней нагрузки '
                                          'и ускорит обработку сообщений. Так же теперь бот может сам напомнить вам о '
                                          'предстоящем дежурстве! ',reply_markup=menu)
    ...
    if message.text != "Протествовать!" and (message.chat.id == ban[1] or message.chat.id == ban[2] or message.chat.id == ban[3] or message.chat.id == ban[4]or message.chat.id == ban[5] or message.chat.id == ban[6] or message.chat.id == ban[7] or message.chat.id == ban[8] or message.chat.id == ban[9] or message.chat.id == ban[10]):
        ...
        if message.text == 'Новости':
            if message.chat.id == 1118648643:
                bot.send_message(message.chat.id,'1. Кирилл уволен!!!')
                bot.send_message(message.chat.id, '2. Кирилл уволен!!!')
                bot.send_message(message.chat.id, '3. Кирилл уволен!!!')
                bot.send_message(message.chat.id, '4. Кирилл уволен!!!')
                bot.send_message(message.chat.id, '5. Кирилл уволен!!!')
            else:
        ...
        if message.text == 'События':
            if message.chat.id == 1118648643:
                bot.send_message(message.chat.id,'Кирилл уволен!!!')
            else:
                bot.send_message(message.chat.id,ev)
        if message.text == 'Дежурство':
            if message.chat.id == 1118648643:
                bot.send_message(message.chat.id, 'ДЕЖУРНЫЕ НА ЭТОЙ НЕДЕЛЕ:')
                bot.send_message(message.chat.id, 'ПН.Лукьянов')
                bot.send_message(message.chat.id, 'ВТ.Лукьянов')
                bot.send_message(message.chat.id, 'СР.Лукьянов')
                bot.send_message(message.chat.id, 'ЧТ.Лукьянов')
                bot.send_message(message.chat.id, 'ПТ.Лукьянов')
                bot.send_message(message.chat.id, 'СБ.Лукьянов')
            else:
                bot.send_message(message.chat.id, 'ДЕЖУРНЫЕ НА ЭТОЙ НЕДЕЛЕ:')
        ...
        if message.text == 'ПН':
            if message.chat.id == 1118648643:
                ...
                for i in range(8):
                    sub = sub + 1
                    bot.send_message(message.chat.id,str(i+1) + '.' + mnk[sub-1])
            else:
                sub = 0
                for i in range(8):
                    sub = sub + 1
            ...
            if message.chat.id == 1118648643:
                ...
                for i in range(8):
                    sub = sub + 1
                    bot.send_message(message.chat.id,str(i+1) + '.' + tuk[sub-1])
            else:
                sub = 0
                for i in range(8):
                    sub = sub + 1
            ...
            if message.chat.id == 1118648643:
                ...
                for i in range(8):
                    sub = sub + 1
                    bot.send_message(message.chat.id,str(i+1) + '.' + wdk[sub-1])
            else:
                sub = 0
                for i in range(8):
                    sub = sub + 1
            ...
            if message.chat.id == 1118648643:
                ...
                for i in range(8):
                    sub = sub + 1
                    bot.send_message(message.chat.id,str(i+1) + '.' + thk[sub-1])
            else:
                sub = 0
                for i in range(8):
                    sub = sub + 1
            ...
            if message.chat.id == 1118648643:
                ...
                for i in range(8):
                    sub = sub + 1
                    bot.send_message(message.chat.id,str(i+1) + '.' + frk[sub-1])
            else:
                sub = 0
                for i in range(8):
                    sub = sub + 1
            ...
            if message.chat.id == 1118648643:
                ...
                for i in range(6):
                    sub = sub + 1
                    bot.send_message(message.chat.id,str(i+1) + '.' + stk[sub-1])
            else:
                sub = 0
                for i in range(6):
                    sub = sub + 1
        ...
        if message.text == 'Биология':
            if message.chat.id == 1118648643:
                bot.send_message(message.chat.id, 'Cпросите у Кирилла!')
            else:
            ...
            if message.chat.id == 1118648643:
                ...
                bot.send_message(message.chat.id,'По математике задали: ' + hw[9])
        ...
        bot.send_message(message.chat.id, 'Вашего id нету всписке,вероятнее всего'
                                          'вы не зарегестрировались в прошлом обновлении,'
                                          'функции регистрации уже не существует.', reply_markup=bm)
